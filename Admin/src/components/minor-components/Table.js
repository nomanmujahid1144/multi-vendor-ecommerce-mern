...
import playAlert from '../../../node_modules/alert-sound-notify/index';
...
export const Table = ({ ordersColumns, approvedOrders, unAssignedOrders, acceptedOrders, completedOrders, ordersData, title, pendingOrders, forceReload, setForceReload, setLoading, type }) => {
    ...
    const rangeSlider = async (value, geometry, index) => {
        document.querySelector('#rangeValue').innerHTML = value;
        setRange(value)

        await axiosInstance.get('/api/v1/order/getalldriversinrange', {
            params: {
                geometry: geometry,
                radius: value
            }
            ...
            .then((res) => {
                ...
                data[index].drivers = res.data.data
                setData(data)
            })
            .catch((err) => {
                            ...
                            <div className="py-3 ">
                                <div className="overflow-x-auto">
                                    <table className="table-auto w-full ">
                                        ...
                                        <tbody className="text-sm  divide-gray-100 z-0">
                                            ...
                                            {!searchText ? <>
                                                ...
                                                {data?.slice(pagesVisited, pagesVisited + itemsPerPage).map((item, index) => (
                                                    <tr key={index}>
                                                        ...
                                                        <td className={`text-left text-xs px-2 py-8 whitespace-nowrap ${index % 2 !== 0 ? 'bg-white' : 'bg-gray-50'}`}>
                                                            <p className={`text-left text-xs `}>{item.restaurnatName}</p>
                                                        </td>
                                                        <td className={`text-left text-xs px-2 py-8 whitespace-nowrap ${index % 2 !== 0 ? 'bg-white' : 'bg-gray-50'}`}>
