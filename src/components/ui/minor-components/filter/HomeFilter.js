...
import restaurantNotFoundIcon from '../../../../assets/empty-product/restaurant.png';
...
export const HomeFilters = () => {
  ...
  const [withInTheLocation, setwithInTheLocation] = useState(false);
  ...
  useEffect(() => {
    ...
    if (Geomatery) {
      if (Object.keys(JSON.parse(Geomatery)).length) {
        ...
        // setFormattedAddress(JSON.parse(Geomatery).formattedAddress)
      }
    }
        ...
        <div>
          {Object?.keys(homeData)?.length > 0 ? 
            ...
            <div className="flex justify-center ">
              <img alt="no image" src={restaurantNotFoundIcon} />
            </div>
              <h6 className="flex justify-center pt-2">
                We're coming soon
              </h6>
              <p className="flex justify-center pt-1">We are always expanding our coverage area. Please check back in the future.
              </p>
            </div>
          }
        </div>
